    ...
    <>
      ...
      <Modal title="Crea tu acta de Integralidad" id="createIntegralityModal">
        <form key={1} className="modal-form" onSubmit={
          ...
          <div className="buttons" style={{ display: 'flex', justifyContent: 'space-around' }}>
            ...
            <button className="btn btn-success w-25" type="submit" name="createIntegralityAct">Crear</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal title="Crear Acta de Reunion" id="createMeetingActModal">
        <form key={2} className="modal-form" onSubmit={
          handleSubmit2((dat, e) => {
            ...
            fetch('/createMeetingAct', {
              method: 'POST',
              body: JSON.stringify({ user: user.username, date: new Date(Date.now()).toLocaleString(), integrality: dat }),
              headers: { 'Content-Type': 'application/json' }
            }).then((res) => {
              res.json().then(async (e) => {
                if (e.createUser) {
      ...
      <Modal title="Editar acta de reunion" id="editMeetingActModal">
        <form key={5} className="modal-form" onSubmit={
          ...
          <div className='form-container'>
            <div className="form-group">
              <textarea className="form-control" rows={12} value={meetingsActs.description} {...register5('meetingEditDescription', { required: true, minLength: 25 })}></textarea>
      ...
      <Modal title="Editar plan de actividades" id="editActivityPlanModal">
        <form key={6} className="modal-form" onSubmit={
          ...
          <div className='form-container'>
            <div className="form-group">
              <textarea className="form-control" rows={12} value={activityPlan.description} {...register6('activityPlanEditDescription', { required: true, minLength: 25 })}></textarea>
